// Enhanced RPO key hook - captura cipher name E key completa (32 bytes para AES-256)
// Usa EVP_CIPHER_CTX_get_key_length e EVP_CIPHER_CTX_get_iv_length
// Compilar: g++ -O2 -shared -fPIC rpo_key_hook_enhanced.cpp -o rpo_key_hook_enhanced.so -ldl
...
typedef struct {
    ...
    char type[32];
    char cipher[128];
    char key[128];      // Até 64 bytes (128 hex chars)
    char iv[64];        // Até 32 bytes (64 hex chars)
    char plaintext[512];
} Event;
...
static FILE* g_json_file = nullptr;
...
static void flush_json() {
    if (!g_json_file || g_event_count == 0) return;
    
    FILE* tmp = fopen("/tmp/rpo_keys_tmp.json", "w");
    if (!tmp) return;
    
    fprintf(tmp, "[\n");
    for (int i = 0; i < g_event_count; i++) {
        Event* e = &g_events[i];
        fprintf(tmp, "  {\"n\": %d, \"type\": \"%s\"", e->n, e->type);
        if (e->cipher[0]) fprintf(tmp, ", \"cipher\": \"%s\"", e->cipher);
        if (e->key[0]) fprintf(tmp, ", \"key\": \"%s\"", e->key);
        if (e->iv[0]) fprintf(tmp, ", \"iv\": \"%s\"", e->iv);
        if (e->plaintext[0]) fprintf(tmp, ", \"plaintext\": \"%s\"", e->plaintext);
        fprintf(tmp, "}%c\n", (i < g_event_count - 1) ? ',' : ' ');
    }
    fprintf(tmp, "]\n");
    fclose(tmp);
    ...
    if (path) rename("/tmp/rpo_keys_tmp.json", path);
}

__attribute__((constructor))
static void init_hook() {
    const char* json_path = getenv("RPO_KEYS_EXPORT");
    const char* path = json_path ? json_path : "/tmp/rpo_keys_enhanced.json";
    
    g_events = (Event*)malloc(g_buffer_size * sizeof(Event));
    g_json_file = fopen(path, "w");
    if (g_json_file) fprintf(g_json_file, "[\n");
    ...
    if (g_libcrypto) {
        ...
        fprintf(stderr, "[rpo_key_hook_enhanced] Hooks:\n");
...
static void add_event(const char* type, const char* cipher, 
    ...
    if (g_json_file) {
        fprintf(g_json_file, "  {\"n\": %d, \"type\": \"%s\"", e->n, e->type);
        if (e->cipher[0]) fprintf(g_json_file, ", \"cipher\": \"%s\"", e->cipher);
        if (e->key[0]) fprintf(g_json_file, ", \"key\": \"%s\"", e->key);
        if (e->iv[0]) fprintf(g_json_file, ", \"iv\": \"%s\"", e->iv);
        if (e->plaintext[0]) fprintf(g_json_file, ", \"plaintext\": \"%s\"", e->plaintext);
        fprintf(g_json_file, "},\n");
        fflush(g_json_file);
    }
    ...
    if (g_event_count % 10 == 0) flush_json();
}
...
int EVP_EncryptInit_ex(void* ctx, void* cipher, void* impl, unsigned char* key, unsigned char* iv) {
    if (g_orig_evpinit) {
        ...
        // Obter tamanhos do contexto
        int key_len = 0;
        int iv_len = 0;
        
        if (g_orig_ctx_get_keylen) {
            key_len = g_orig_ctx_get_keylen(ctx);
        }
        if (g_orig_ctx_get_ivlen) {
            iv_len = g_orig_ctx_get_ivlen(ctx);
        }
        
        // Capturar key completa
        ...
        fprintf(stderr, "[rpo_key_hook_enhanced] InitEx: cipher=%s key_len=%d iv_len=%d\n", 
